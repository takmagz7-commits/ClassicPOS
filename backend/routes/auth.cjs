...
const databaseReadinessMiddleware = require('../middleware/databaseReadiness.cjs');
...
router.post('/signup', databaseReadinessMiddleware, registrationLockMiddleware, activityLogger('auth', 'Signup'), validateRequest(signupSchema), async (req, res) => {
  try {
    const { 
      email, 
      password, 
...
router.post('/login', databaseReadinessMiddleware, activityLogger('auth', 'Login'), validateRequest(loginSchema), async (req, res) => {
  try {
...
router.post('/complete-mfa-setup', authMiddleware, activityLogger('auth', 'Complete MFA Setup'), async (req, res) => {
  try {
    ...
    const insertBackupCode = db.prepare('INSERT INTO mfa_backup_codes (id, user_id, code_hash, created_at) VALUES (?, ?, ?, ?)');
    for (const codeData of hashedBackupCodes) {
      insertBackupCode.run(codeData.id, codeData.user_id, codeData.code_hash, codeData.created_at);
    }
...
router.post('/verify-mfa', databaseReadinessMiddleware, activityLogger('auth', 'Verify MFA'), validateRequest(verifyMfaSchema), async (req, res) => {
  try {
    ...
    if (backupCode) {
      const db = getDatabase();
      const userBackupCodes = db.prepare('SELECT id, code_hash FROM mfa_backup_codes WHERE user_id = ?').all(userId);
      ...
      for (const codeData of userBackupCodes) {
        const isMatch = await bcrypt.compare(backupCode, codeData.code_hash);
        if (isMatch) {
      ...
      db.prepare('DELETE FROM mfa_backup_codes WHERE id = ?').run(matchedCodeId);
...
router.post('/complete-mfa-login', databaseReadinessMiddleware, validateRequest(completeMfaLoginSchema), async (req, res) => {
  try {
...
function recordPinAttempt(ipAddress) {
  const now = Date.now();
  const entry = pinAttemptTracker.get(ipAddress) || { attempts: 0, firstAt: now };
  
  if (now - entry.firstAt > PIN_RATE_LIMIT_WINDOW_MS) {
    entry.attempts = 1;
    entry.firstAt = now;
  } else {
    entry.attempts += 1;
  ...
  pinAttemptTracker.set(ipAddress, entry);
  return entry;
}

function resetPinAttempts(ipAddress) {
  pinAttemptTracker.delete(ipAddress);
}

function isPinRateLimited(ipAddress) {
  const entry = pinAttemptTracker.get(ipAddress);
  if (!entry) return false;
  
  const now = Date.now();
  return entry.attempts >= MAX_PIN_ATTEMPTS && (now - entry.firstAt) < PIN_RATE_LIMIT_WINDOW_MS;
}

router.post('/pin-login', databaseReadinessMiddleware, activityLogger('auth', 'PIN Login'), validateRequest(pinLoginSchema), async (req, res) => {
  try {
    ...
    if (!matchedUser) {
      recordPinAttempt(ipAddress);
      const attempts = pinAttemptTracker.get(ipAddress);
      
      return res.status(401).json({ 
        error: 'Invalid PIN',
        message: 'Invalid PIN code or user is inactive',
        attemptsRemaining: Math.max(0, MAX_PIN_ATTEMPTS - attempts.attempts)
      });
    }

    resetPinAttempts(ipAddress);
    ...
    logger.info(`PIN login successful for user: ${user.email}`);
...
router.post('/logout', authMiddleware, activityLogger('auth', 'Logout'), (req, res) => {
  try {
    ...
    if (req.user && req.user.jti) {
      ...
      if (token) {
        try {
          ...
          db.prepare('INSERT OR IGNORE INTO revoked_tokens (jti, user_id, revoked_at, expires_at) VALUES (?, ?, ?, ?)')
            .run(req.user.jti, req.user.id, new Date().toISOString(), expiresAt);
          
          logger.info(`Token revoked for user: ${req.user.email}, jti: ${req.user.jti}`);
        } catch (error) {
...
router.get('/system-status', databaseReadinessMiddleware, (req, res) => {
  try {
