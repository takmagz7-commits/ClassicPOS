...
const paymentMethodToDb = (method) => ({
  id: method.id,
  name: method.name,
...
const dbToPaymentMethod = (dbMethod) => ({
  id: dbMethod.id,
  name: dbMethod.name,
...
// Stock Adjustment conversion
const stockAdjustmentToDb = (adj) => ({
  id: adj.id,
...
const dbToStockAdjustment = (dbAdj) => ({
  id: dbAdj.id,
...
// Transfer of Goods conversion
const transferToDb = (transfer) => ({
  id: transfer.id,
...
// Inventory History conversion
const inventoryHistoryToDb = (entry) => ({
  id: entry.id,
  date: entry.date,
  type: entry.type,
...
const dbToInventoryHistory = (dbEntry) => ({
  id: dbEntry.id,
  date: dbEntry.date,
  type: dbEntry.type,
...
const attendanceToDb = (attendance) => ({
  id: attendance.id,
...
const dbToAttendance = (dbAttendance) => ({
  id: dbAttendance.id,
...
const permissionToDb = (permission) => ({
  id: permission.id,
...
const dbToPermission = (dbPermission) => ({
  id: dbPermission.id,
...
const rolePermissionToDb = (rp) => ({
  id: rp.id,
  role_id: rp.roleId,
  permission_id: rp.permissionId,
});

const dbToRolePermission = (dbRp) => ({
  id: dbRp.id,
  roleId: dbRp.role_id,
...
const chartOfAccountToDb = (account) => ({
  id: account.id,
  ...
  is_active: boolToInt(account.isActive),
});

const dbToChartOfAccount = (dbAccount) => ({
  id: dbAccount.id,
  accountCode: dbAccount.account_code,
  accountName: dbAccount.account_name,
  accountType: dbAccount.account_type,
  accountCategory: dbAccount.account_category,
  parentAccountId: dbAccount.parent_account_id || undefined,
  description: dbAccount.description || undefined,
  isActive: intToBool(dbAccount.is_active),
});

// Journal Entry conversion
const journalEntryToDb = (entry) => ({
  id: entry.id,
  ...
  description: entry.description,
  posted_by_user_id: entry.postedByUserId || null,
  posted_by_user_name: entry.postedByUserName || null,
  is_posted: boolToInt(entry.isPosted),
});

const dbToJournalEntry = (dbEntry) => ({
  id: dbEntry.id,
  entryDate: dbEntry.entry_date,
  entryNumber: dbEntry.entry_number,
  referenceType: dbEntry.reference_type || undefined,
  referenceId: dbEntry.reference_id || undefined,
  description: dbEntry.description,
  postedByUserId: dbEntry.posted_by_user_id || undefined,
  postedByUserName: dbEntry.posted_by_user_name || undefined,
  isPosted: intToBool(dbEntry.is_posted),
  lines: [],
});

// Journal Entry Line conversion
const journalEntryLineToDb = (line) => ({
  id: line.id,
...
const dbToJournalEntryLine = (dbLine) => ({
  id: dbLine.id,
  journalEntryId: dbLine.journal_entry_id,
...
const dbToBankAccount = (dbAccount) => ({
  id: dbAccount.id,
  accountName: dbAccount.account_name,
  bankName: dbAccount.bank_name,
  accountNumber: dbAccount.account_number,
  accountType: dbAccount.account_type,
  openingBalance: dbAccount.opening_balance,
...
module.exports = {
  ...
  transferToDb,
  dbToTransfer,
  inventoryHistoryToDb,
  dbToInventoryHistory,
  userToDb,
  dbToUser,
  attendanceToDb,
  dbToAttendance,
