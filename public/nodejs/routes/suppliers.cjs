...
router.delete('/:id', (req, res) => {
  try {
    ...
    if (poCount > 0) {
      logger.warn(`Cannot delete supplier ${supplierId}: has ${poCount} related purchase orders`);
      return res.status(409).json({ 
        error: 'Cannot delete supplier',
        message: `This supplier has ${poCount} related purchase order(s). Please delete or reassign those first.`
      });
    }
    ...
    if (grnCount > 0) {
      logger.warn(`Cannot delete supplier ${supplierId}: has ${grnCount} related GRNs`);
      return res.status(409).json({ 
        error: 'Cannot delete supplier',
        message: `This supplier has ${grnCount} goods received note(s). Please delete or reassign those first.`
      });
    }
